    ...
    <div className="max-w-7xl mx-auto px-4 py-8 font-comic relative">
      ...
      <section className="mb-12">
        <div className="bg-yellow-200 p-8 comic-border relative z-20">
          <div className="flex flex-col md:flex-row items-center justify-between mb-8 border-b-4 border-black pb-4 gap-4">
            <h3 className="text-4xl font-bangers text-black uppercase tracking-widest flex items-center gap-4">
              <span className="bg-black text-yellow-200 px-3 py-1 text-sm font-black rotate-3">ACTIVE SAGA:</span>
              {activePack.title}
            </h3>
      ...
      <section className="min-h-[400px]">
        {!storyResult && !isGenerating && (
          <div className="h-full flex items-center justify-center border-8 border-dashed border-gray-400 rounded-3xl bg-gray-50 p-24">
            <div className="text-center">
              <div className="text-9xl mb-8 animate-bounce">🍝</div>
              <h3 className="text-5xl font-bangers text-gray-400 tracking-tighter uppercase">Waiting for inputs...</h3>
              <p className="text-gray-400 mt-4 text-2xl font-bold italic uppercase">Your Hyperrealistic Saga for Walter will appear here!</p>
            </div>
          </div>
        )}

        {isGenerating && (
          <div className="h-full flex flex-col items-center justify-center border-8 border-black bg-white p-12 py-24 text-center relative overflow-hidden halftone comic-border">
            <div className="z-10 bg-white/95 p-12 border-8 border-black shadow-2xl animate-in zoom-in duration-300 max-w-2xl w-full">
              <div className="text-center mb-8">
                <div className="text-[120px] mb-4 leading-none inline-block p-10 bg-yellow-300 border-8 border-black rounded-full shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
                  {BRAINROT_LOADER_CHARS[loaderIndex].icon}
                </div>
              </div>
              <h3 className="text-5xl font-bangers text-black mb-3 uppercase tracking-widest">{BRAINROT_LOADER_CHARS[loaderIndex].name}</h3>
              <p className="text-2xl font-black text-red-600 italic uppercase mb-12 leading-tight">"{BRAINROT_LOADER_CHARS[loaderIndex].desc}"</p>
              <div className="w-full h-10 bg-gray-200 border-4 border-black relative mb-6">
                <div className="bg-green-600 h-full transition-all duration-500" style={{ width: `${((verifiedCount || 0) / 6) * 100}%` }}></div>
              </div>
              <p className="font-bangers text-3xl animate-pulse text-black uppercase tracking-tight">{loadingStep}</p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-12 animate-in fade-in zoom-in duration-1000 pb-24">
            <div className="bg-white p-8 comic-border">
              <div className="flex flex-wrap items-center justify-between gap-8 mb-6">
                <div className="flex items-center gap-6">
                   <div className={`w-20 h-20 rounded-full flex items-center justify-center border-4 border-black ${isSpeaking ? 'bg-yellow-400 animate-bounce' : 'bg-green-500 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]'}`}>
                      <span className="text-5xl">{isSpeaking ? '🤌' : '🎙️'}</span>
                   </div>
                   <div>
                     <h4 className="font-bangers text-4xl text-black leading-none uppercase">COMIC NARRATOR</h4>
                     <p className="text-lg font-black text-gray-500 uppercase italic">Hyperrealistic Drama for Walter Aulson</p>
                   </div>
                </div>
                <div className="flex items-center gap-4">
                  <button 
                    onClick={isSpeaking ? handleStopVoice : handlePlayVoice}
                    className={`px-10 py-5 border-4 border-black font-bangers text-3xl shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:translate-y-0.5 hover:shadow-none transition-all active:scale-95 ${isSpeaking ? 'bg-red-400' : 'bg-green-400'}`}
                  >
                    {isSpeaking ? 'STOP NARRATION' : 'PLAY STORY'}
                  </button>
                  <button 
                    onClick={() => setShowVoiceHelp(true)}
                    className="p-5 border-4 border-black bg-blue-100 hover:bg-blue-200 transition-colors shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
                  >
                    <span className="text-2xl">⚙️</span>
                  </button>
                </div>
              </div>
              <div className="p-8 bg-gray-50 border-4 border-dashed border-gray-300">
                 <p className="text-2xl font-bold italic leading-relaxed text-black">"{storyResult.fullScript}"</p>
              </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-8 border-black bg-black gap-2 overflow-hidden shadow-[32px_32px_0px_0px_rgba(0,0,0,1)]">
              {storyResult.panels.map((panel, idx) => (
                <div key={idx} className="bg-white flex flex-col group relative overflow-hidden border-black">
                  <div className="relative overflow-hidden bg-gray-200 aspect-square">
                     ...
                     <div className="absolute top-0 left-0 bg-black text-white px-6 py-3 font-bangers text-3xl border-r-8 border-b-8 border-black z-10 shadow-xl">{idx + 1}</div>
                  </div>
                  <div className="p-6 bg-white border-t-8 border-black min-h-[100px] flex items-center justify-center relative z-20">
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-8 pt-10">
              <button 
                ...
                className="bg-green-600 text-white px-16 py-6 font-bangers text-4xl border-8 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all active:scale-95 uppercase tracking-widest"
              >
                PRINT COMIC-INI! 🖨️
              </button>
              <button 
                onClick={handleNewBrainrot} 
                className="bg-white text-black px-16 py-6 font-bangers text-4xl border-8 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] hover:translate-x-1 hover:translate-y-1 hover:shadow-none transition-all active:scale-95 uppercase tracking-widest"
              >
                NEW BRAINROT! ✨
              </button>
            </div>
      ...
      {showVoiceHelp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          <div className="bg-white comic-border p-10 max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-in zoom-in duration-200">
            <h3 className="text-5xl font-bangers text-red-600 mb-6 uppercase tracking-tight">Install the Italian Voice! 🇮🇹</h3>
            <p className="mb-8 font-bold text-xl text-gray-900 leading-tight">
            ...
            <button 
              ...
              className="mt-12 w-full bg-black text-white py-6 font-bangers text-3xl hover:bg-gray-800 transition-colors uppercase shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]"
            >
              GOT IT-INI!
            </button>
          </div>
      ...
      <footer className="mt-20 pt-10 border-t-8 border-black text-center pb-20">
        <h4 className="font-luckiest text-5xl text-red-600 tracking-widest mb-2 uppercase">© WALTER AULSON • {new Date().getFullYear()}</h4>
        <p className="text-sm font-black uppercase tracking-[0.3em] text-gray-500 italic">Sequential Hyperrealistic verified Hybrid Engine</p>
      </footer>
    </div>
  );
};
